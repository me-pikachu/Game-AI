...
#include <omp.h>
    ...
    #define CONSTANT_C1 50 // constant representing exploration and exploitation
#endif
#ifndef SIMULATION_DEPTH // if it not defined by the compiler at compilation time
    #define SIMULATION_DEPTH 3 // the depth of alpha beta
#endif
#ifndef MAX_EVAL_TIME // if it not defined by the compiler at compilation time
    #define MAX_EVAL_TIME 5000 // time in milliseconds
#endif
...
alphabetaeval alphabeta(position& curpos, byte depth, const byte& max_depth, int alpha = -GAMEOVEREVAL, int beta = GAMEOVEREVAL){
    if (depth == max_depth){
        // we are at root node
    ...
    if (newpos.tomove & 1){
        ...
        for (int i=0; i<moves.size(); ++i){
            ...
            curval = alphabeta(moves[i].second, depth+1, max_depth, alpha, beta);
    ...
    *res = moves[0];
    ...
    if (root.tomove & 1){
        ...
        for (int i=0; i<moves.size(); ++i){
            ...
            curval = alphabeta(moves[i].second, 1, max_depth, alpha, beta);
...
int simulate(position curpos, bool ismax){
    ...
    return alphabeta(curpos, 0, SIMULATION_DEPTH, ismax).eval;
}

int max_reached_depth = 0;

#ifdef showdepth
int traverse(node& curnode, std::unordered_set<position, position_hash>& prev_pos, int cur_reached_depth=0){
    ...
    for (auto& i:*child){
        if (i.n == 0){
            ...
            if (cur_reached_depth > max_reached_depth){
                ...
                std::cout << "Depth " << cur_reached_depth << " reached" << std::endl;
            }

            int eval = simulate(*(static_cast<position*>(i.child)), i.ismax);
            i.total_value = eval;
            i.n = 1;
            curnode.n++;
    ...
    if (selnode->n == 1){
        ...
        // this node expansion would not make sense if it is already present in the tree (repetitive moves)
        if (prev_pos.find(*(static_cast<position*>(selnode->child))) != prev_pos.end()){
        ...
        int eval = traverse(*selnode, cur_reached_depth+1);
        curnode.n++;
        curnode.total_value += eval;
        prev_pos.erase(thispos);
        return eval;
...
#else
int traverse(node& curnode, std::unordered_map<position, byte, position_hash>& prev_pos){
    ...
    if (selnode->n == 1){
        ...
        int eval = traverse(*selnode, prev_pos);
        curnode.n++;
        curnode.total_value = eval;
        //prev_pos.erase(thispos);
        return eval;
...
evaluation MCTS(position curpos){
    ...
    while (root.n < MAX_ROOT_VISIT && (convert_to_milli(timenow-start)) < MAX_EVAL_TIME){
        ...
        prev_pos[curpos] = 0;
        #ifdef showdepth
        traverse(root, prev_pos, 0); // root is at depth 0
        #else
        traverse(root, prev_pos);
        #endif
    }
...
int main(){
    ...
    while (toplay){
        if (game.tomove){
            ...
            game = res.second;
            std::cout << "Alphabeta Evaluation: " << round((double)output.eval/1000, 3) << std::endl;
        } else {
